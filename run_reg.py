...
# Code taken from https://github.com/swen128/bert-sts/blob/master/run_reg.py
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, val):
...
def convert_single_example(ex_index, example, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf.logging.info("val: %s" % (example.label))
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, pred_vals) = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, vals,
         use_one_hot_embeddings)
    ...
    d = tf.data.Dataset.from_tensor_slices({
        ...
        "segment_ids":
            tf.constant(
                ...
                dtype=tf.int32),
        "vals":
            tf.constant(all_vals, shape=[num_examples], dtype=tf.float32),
    })
